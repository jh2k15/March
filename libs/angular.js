 ...
 * - `{void}` `put({string} key, {*} value)` 鈥� Puts a new key-value pair into the cache.
 ...
 * **Methods**
 *
 * - `then(successCallback, errorCallback)` 鈥� regardless of when the promise was or will be resolved
...
function $RouteProvider(){
   ...
   *    Object properties:
   *
   *    - `controller` 鈥� `{function()=}` 鈥� Controller fn that should be associated with newly
   *      created scope.
   *    - `template` 鈥� `{string=}` 鈥�  html template as a string that should be used by
   *      {@link ng.directive:ngView ngView} or
   *      {@link ng.directive:ngInclude ngInclude} directives.
   *      this property takes precedence over `templateUrl`.
   *    - `templateUrl` 鈥� `{string=}` 鈥� path to an html template that should be used by
...
function $HttpProvider() {
  ...
  this.$get = ['$httpBackend', '$browser', '$cacheFactory', '$rootScope', '$q', '$injector',
     ...
     *    - **params** 鈥� `{Object.<string|Object>}` 鈥� Map of strings or objects which will be turned to
     *      `?key1=value1&key2=value2` after the url. If the value is not a string, it will be JSONified.
     *    - **data** 鈥� `{string|Object}` 鈥� Data to be sent as the request message data.
     *    - **headers** 鈥� `{Object}` 鈥� Map of strings representing HTTP headers to send to the server.
     *    - **transformRequest** 鈥� `{function(data, headersGetter)|Array.<function(data, headersGetter)>}` 鈥�
     *      transform function or an array of such functions. The transform function takes the http
     *      request body and headers and returns its transformed (typically serialized) version.
     *    - **transformResponse** 鈥� `{function(data, headersGetter)|Array.<function(data, headersGetter)>}` 鈥�
     *      transform function or an array of such functions. The transform function takes the http
     ...
     *      caching.
     *    - **timeout** 鈥� `{number}` 鈥� timeout in milliseconds.
     ...
     *
     *   - **data** 鈥� `{string|Object}` 鈥� The response body transformed with the transform functions.
 ...
 * @example
   <doc:example>
     <doc:source>
       ...
       Phone only <input ng-model="search.phone"氓><br>
 ...
 *  forms, where:
 *
 *  - keys are validation tokens (error names) 鈥� such as `REQUIRED`, `URL` or `EMAIL`),
 ...
 *
 * * Model 鈥� The Model is data in scope properties; scopes are attached to the DOM.
 * * View 鈥� The template (HTML with data bindings) is rendered into the View.
 * * Controller 鈥� The `ngController` directive specifies a Controller class; the class has
 ...
 *
 *   * `$index` 鈥� `{number}` 鈥� iterator offset of the repeated element (0..length-1)
 *   * `$first` 鈥� `{boolean}` 鈥� true if the repeated element is first in the iterator.
 *   * `$middle` 鈥� `{boolean}` 鈥� true if the repeated element is between the first and last in the iterator.
 *   * `$last` 鈥� `{boolean}` 鈥� true if the repeated element is last in the iterator.
 *
 *
 * @element ANY
 * @scope
